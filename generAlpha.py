...
for row in range(longest_row -4): #Row -4 because son marjlarin 4 ceyrek oncesi yok
    ...
    for file_name, date in sorted_pairs.items():
        ...
        if row in data_frame.index:
            ...
            #Checking Criterias
            if (in_query['Net Kar Marjı Yıllık'] > 0.1).any():
...
df.to_excel('/home/gun/Documents/Siralamalar/DefaultAgirlik/BilancoToBilanco/SingleRatioSorts/NKY.xlsx', sheet_name="NKY Only", index=True)
# show(df)
